                ...
                <div class="row align-items-center mt-3 mb-4">
                    <div class="col-md-6">
                        <div class="d-flex align-items-center">
                            <a href="{{ route('sales.approval') }}" class="btn btn-light border me-3 shadow-sm rounded-circle"
                                style="width: 40px; height: 40px; display: flex; align-items: center; justify-content: center;">
                                <i class="fa fa-arrow-left"></i>
                            </a>
                            <div>
                                <h4 class="fw-bold text-dark mb-0">Review Persetujuan</h4>
...
@push('js')
    <script>
        ...
        async function loadInvoiceDetail() {
            try {
                // Fetch with sum payment
                const response = await fetch(
                    `${API_URL}/acc-admin?search=${invoiceId}`
                ); // Using search to narrow down to current ID in the Acc-Admin results structure if needed, or better, fetch directly if allowed.
                // Re-fetch directly to get latest status and payment sum correctly
                ...
                if (directResult.success) {
                    ...
                    window.location.href = '{{ route('sales.approval') }}';
                }
            } catch (e) {
                console.error(e);
        ...
        function renderInvoice(data) {
            ...
            const hasPayment = (data.payment && data.payment.length > 0);
            const approval = data.approvals && data.approvals.length > 0 ? data.approvals[0] : null;
            const status = approval ? approval.status : 'pending';

            if (hasPayment) {
                const receiptId = data.payment[0].id;
                header.innerHTML = `
                    <div class="d-flex align-items-center text-success fw-bold small">
                        <i class="fa fa-check-double me-1"></i> Kuitansi Terbit
                    </div>
                    <a href="/sales-receipt/${receiptId}" class="btn btn-primary fw-bold px-4 shadow-sm">
                        <i class="fa fa-receipt me-1"></i> Lihat Kuitansi
                    </a>
                `;
            } else if (status === 'approved') {
                header.innerHTML = `
                    <button class="btn btn-warning fw-bold px-4 shadow-sm" onclick="withdraw()">
                        <i class="fa fa-undo me-1"></i> Batalkan Persetujuan
                    </button>
                    <button class="btn btn-outline-success fw-bold px-3 ms-2 disabled" disabled>
                        <i class="fa fa-check-circle"></i> Status: Disetujui
                    </button>
                `;
            } else if (status === 'rejected') {
                header.innerHTML = `
                    <button class="btn btn-warning fw-bold px-4 shadow-sm" onclick="withdraw()">
                        <i class="fa fa-undo me-1"></i> Batalkan Penolakan
                    </button>
                    <button class="btn btn-outline-danger fw-bold px-3 ms-2 disabled" disabled>
        ...
        async function approveInvoice() {
            if (!await macConfirm('Konfirmasi Persetujuan', 'Apakah Anda yakin ingin MENYETUJUI invoice ini?', {
                    confirmText: 'Setujui',
                    confirmType: 'success'
                })) return;

            try {
                const response = await fetch(`${API_URL}/${invoiceId}/approve`, {
                    method: 'POST',
                    headers: {
        ...
        async function rejectInvoice() {
            if (!await macConfirm('Konfirmasi Penolakan', 'Apakah Anda yakin ingin MENOLAK invoice ini?', {
                    confirmText: 'Tolak',
                    confirmType: 'danger'
                })) return;

            try {
                const response = await fetch(`${API_URL}/${invoiceId}/reject`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
        ...
        async function withdraw() {
            if (!await macConfirm('Tarik Keputusan', 'Batalkan keputusan persetujuan/penolakan invoice ini?', {
                    confirmText: 'Tarik',
                    confirmType: 'warning'
                })) return;

            try {
                const response = await fetch(`${API_URL}/${invoiceId}/withdraw`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
