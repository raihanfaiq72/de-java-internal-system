                ...
                <div class="row">
                    <div class="col-sm-12">
                        <div class="page-title-box d-md-flex justify-content-between align-items-center">
                            <h4 class="page-title">Persetujuan Invoice</h4>
...
@push('js')
    ...
    <script>
        if (!window.financeApp) {
            window.financeApp = {
                API_URL: '/api/invoice-api/acc-admin',
        ...
        function renderInvoiceList(items) {
            ...
            if (items.length === 0) {
                tbody.innerHTML =
                    '<tr><td colspan="6" class="text-center p-4">Tidak ada invoice yang menunggu persetujuan.</td></tr>';
                return;
            }
            ...
            items.forEach((item, index) => {
                ...
                row.onclick = (e) => {
                    if (!e.target.closest('.action-area')) {
                        window.location.href = `/sales/approval/${item.id}`;
                    }
                };

                const hasPayment = (item.payment_sum_jumlah_bayar || 0) > 0;
                const approval = item.approvals && item.approvals.length > 0 ? item.approvals[0] : null;
                const status = approval ? approval.status : 'pending';

                let statusBadge = '';
                if (hasPayment) {
                    statusBadge = '<span class="badge bg-success-subtle text-success border border-success-subtle px-2">Kuitansi Terbit</span>';
                } else if (status === 'approved') {
                    statusBadge = '<span class="badge bg-primary-subtle text-primary border border-primary-subtle px-2">Disetujui</span>';
                } else if (status === 'rejected') {
                    statusBadge = '<span class="badge bg-danger-subtle text-danger border border-danger-subtle px-2">Ditolak</span>';
                } else {
                    statusBadge = '<span class="badge bg-warning-subtle text-warning border border-warning-subtle px-2">Menunggu</span>';
                }
        ...
        async function handleApproveToggle(id, currentStatus) {
            if (currentStatus === 'approved') {
                if (!await macConfirm('Tarik Persetujuan', 'Batalkan persetujuan invoice ini?', { confirmText: 'Tarik', confirmType: 'primary' })) return;
                performAction(id, 'withdraw');
            } else {
                if (!await macConfirm('Konfirmasi Persetujuan', 'Setujui invoice ini?', { confirmText: 'Setujui', confirmType: 'success' })) return;
                performAction(id, 'approve');
            }
        }

        async function handleRejectToggle(id, currentStatus) {
            if (currentStatus === 'rejected') {
                if (!await macConfirm('Tarik Penolakan', 'Batalkan penolakan invoice ini?', { confirmText: 'Tarik', confirmType: 'primary' })) return;
                performAction(id, 'withdraw');
            } else {
