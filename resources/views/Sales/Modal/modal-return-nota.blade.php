        ...
        <div class="modal-content border-0" style="background-color: #f8fafc;">
            ...
            <div class="modal-header bg-white border-bottom px-4 py-3 sticky-top shadow-sm">
                ...
                <div class="ms-auto d-flex align-items-center gap-3">
                    ...
                    <button type="button" class="btn btn-danger px-4 fw-bold shadow-sm" onclick="saveReturnNota()">
                ...
                <form id="returnNotaForm" class="h-100">
                    ...
                    <div class="row g-0 h-100">
                            ...
                            <div class="alert alert-warning border-0 shadow-sm rounded-3 mb-4 d-flex align-items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <div class="small text-muted">
                                        Nota return digunakan untuk mengoreksi kesalahan cetak nota — misalnya jumlah barang atau harga yang salah.
                                        Stok akan dikembalikan dan jurnal akuntansi akan dibuat otomatis (Debit Retur Penjualan, Kredit Piutang Usaha).
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="card-body p-4">
                                    <div class="row g-3">
                                        <div class="col-md-6">
                                            ...
                                            <div class="form-control bg-light text-primary fw-bold" id="rn_display_original">-</div>
                                        </div>
                                        <div class="col-md-3">
                                            <label class="f-label mb-1">Nomor Return <span class="text-danger">*</span></label>
                                            <input type="text" id="rn_nomor_invoice" class="form-control f-input fw-bold bg-light text-danger"
                                                placeholder="Contoh: RTR-001">
                                        </div>
                                        <div class="col-md-3">
                                            <label class="f-label mb-1">Tanggal Return <span class="text-danger">*</span></label>
                            ...
                            <div class="card border-0 shadow-sm rounded-3 overflow-hidden mb-4">
                                <div class="card-header bg-white py-3 px-4 border-bottom d-flex justify-content-between align-items-center">
                                    ...
                                    <div class="d-flex gap-2">
                                        <button type="button" class="btn btn-outline-secondary btn-sm fw-bold px-3 rounded-pill"
                                ...
                                <div class="table-responsive">
                                    <table class="table align-middle mb-0 table-hover" id="rnItemTable">
                                        ...
                                        <tfoot class="bg-light">
                                            <tr id="rnEmptyRow">
                                                <td colspan="8" class="text-center py-2 text-muted small fst-italic">
                                                    Klik "Salin dari Nota Asal" atau "Tambah Baris" untuk memasukkan item
                                                </td>
                                            </tr>
                                        </tfoot>
                                    </table>
                        ...
                        <div class="col-lg-3 bg-white border-start h-100 d-flex flex-column shadow-sm z-1">
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <div class="input-group input-group-sm">
                                            <select id="rn_diskon_tambahan_tipe" class="form-select border-end-0 text-dark"
                                                style="max-width: 85px;" onchange="rnCalculateTotal()">
                                                <option value="Fixed">Rp</option>
                                                <option value="Percentage">%</option>
                                            </select>
                                            <input type="text" id="rn_diskon_tambahan" class="form-control text-end" value="0"
                                                oninput="rnCalculateTotal()">
                                        </div>
                                    </div>
                                    <div class="mb-3">
                                        <label class="text-muted small d-block mb-1">Biaya Lain-lain</label>
                                        <input type="text" id="rn_biaya_lain" class="form-control form-control-sm text-end" value="0"
                                            oninput="rnCalculateTotal()">
                                    </div>
                                </div>
                            ...
                            <div class="p-3 rounded bg-danger bg-opacity-10 border border-danger border-opacity-25 m-3">
...
<template id="rnProductRowTemplate">
    <tr class="border-bottom border-light rn-product-row">
        ...
        <td class="py-3">
            <input type="number" class="form-control form-control-sm text-center rn-prod-qty" value="1"
                min="0.01" step="0.01" oninput="rnCalculateTotal()">
        </td>
        <td class="py-3 text-center">
            <span class="badge bg-light text-secondary border rn-prod-unit">-</span>
        </td>
        <td class="py-3">
            <input type="text" class="form-control form-control-sm text-end rn-prod-price rupiah-input-rn" value="0">
        </td>
        <td class="py-3">
        ...
        <td class="py-3">
            <input type="text" class="form-control form-control-sm text-end rn-prod-disc rupiah-input-rn" value="0">
        </td>
        <td class="py-3 text-end pe-4 fw-bold text-dark rn-prod-subtotal">Rp 0</td>
        <td class="py-3 text-center">
            <button type="button" class="btn btn-link text-danger p-0 rn-btn-remove">
...
<script>
    ...
    // ─── Open Modal ───────────────────────────────────────────────────────────
    function openReturnNotaModal(invoiceId, nomorInvoice) {
        ...
        document.getElementById('rn_nomor_invoice').value = `RTR-${ymd}-${invoiceId}`;
    ...
    function rnAddRow(data = {}) {
        ...
        if (data.qty)      row.querySelector('.rn-prod-qty').value = data.qty;
        if (data.harga)    row.querySelector('.rn-prod-price').value = formatRupiahInput(data.harga);
        if (data.discTipe) row.querySelector('.rn-prod-disc-tipe').value = data.discTipe;
        if (data.disc)     row.querySelector('.rn-prod-disc').value = formatRupiahInput(data.disc);
        if (data.desc)     row.querySelector('.rn-prod-desc').value = data.desc;
        if (data.prodId)   row.querySelector('.rn-prod-id').value = data.prodId;
    ...
    async function saveReturnNota() {
        ...
        rows.forEach(row => {
            ...
            const namaProdukManual = selectedOpt ? selectedOpt.textContent.split(' (')[0] : '-';

            items.push({
                produk_id: prodId || null,
                nama_produk_manual: prodId ? null : namaProdukManual,
        ...
        try {
            const btn = document.querySelector('#returnNotaModal .btn-danger[onclick="saveReturnNota()"]');
            btn.disabled = true;
            btn.innerHTML = '<i class="fa fa-spinner fa-spin me-2"></i>Menyimpan...';

            const res = await fetch('{{ url('api/invoice-return-api') }}', {
                method: 'POST',
                headers: {
            ...
            btn.disabled = false;
            btn.innerHTML = '<i class="fa fa-save me-2"></i>Simpan Return';

            if (result.success) {
                const returnId = result.data.id;
                bootstrap.Modal.getInstance(document.getElementById('returnNotaModal')).hide();
                ...
                // Reload invoice detail to show updated info
            ...
            console.error(e);
            alert('Terjadi kesalahan saat menyimpan nota return.');
            const btn = document.querySelector('#returnNotaModal .btn-danger[onclick="saveReturnNota()"]');
            if (btn) {
                btn.disabled = false;
                btn.innerHTML = '<i class="fa fa-save me-2"></i>Simpan Return';
