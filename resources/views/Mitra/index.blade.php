            ...
            <div class="card shadow-sm border-0 rounded-3 overflow-hidden">
                <div class="card-header bg-white border-bottom py-0 px-4">
                    <ul class="nav nav-tabs nav-tabs-finance" role="tablist">
                        <li class="nav-item"><a class="nav-link active fw-bold py-3" data-bs-toggle="tab" href="#mitra-active">Mitra Aktif</a></li>
                        <li class="nav-item"><a class="nav-link fw-bold py-3" data-bs-toggle="tab" href="#mitra-deleted">Sampah</a></li>
                    </ul>
                </div>
                            ...
                            <div class="row g-2 mb-4 p-3 rounded-3 align-items-end bg-light border">
                                <div class="col-lg-4">
                                    ...
                                    <div class="input-group input-group-finance">
                                        ...
                                        <input type="text" id="filter-search" class="form-control border-start-0 ps-0 shadow-none" placeholder="Nama, No. Mitra, atau Email...">
                                    </div>
                                </div>
                                <div class="col-lg-2">
...
<style>
    ...
    /* Table Styling */
    #mitraTable thead th { border-bottom: 1px solid #e2e8f0; border-top: 1px solid #e2e8f0; background: #f8fafc; padding-top: 12px; padding-bottom: 12px; }
    #mitraTable tbody td { font-size: 13px; color: #334155; padding-top: 12px; padding-bottom: 12px; border-bottom: 1px solid #f1f5f9; }
    ...
    .f-supplier { background: #fee2e2; color: #991b1b; } /* Red for expense related */
    .f-client { background: #dcfce7; color: #166534; } /* Green for income related */
    .f-both { background: #e0f2fe; color: #075985; } /* Blue for both */
...
<script>
    ...
    async function loadMitraData(url = window.financeApp.MITRA_API_URL || '/api/mitra-api') {
        const tbody = document.getElementById('mitraTableBody');
        tbody.innerHTML = '<tr><td colspan="7" class="text-center p-5"><div class="spinner-border spinner-border-sm text-primary"></div><p class="mt-2 text-muted small mb-0">Memuat data mitra...</p></td></tr>';

        try {
            let finalUrl = (typeof url === 'string' && url.includes('/')) ? url : (window.financeApp.MITRA_API_URL || '/api/mitra-api');
            
            // Check if search or filter overrides
            const searchVal = document.getElementById('filter-search').value;
            const typeVal = document.getElementById('filter-type').value;

            // Mitra API search is distinct: /api/mitra-api/search/{value} vs /api/mitra-api?page=X
            // If search is active, use search endpoint OR standard endpoint with params if supported.
            // Based on MitraController: index() returns all latets, search($val) returns search results.
            // But standard index() doesn't seem to take filters in the controller shown?
            // Wait, looking at MitraController.php, index() uses paginate(10) directly on Mitra::latest().
            // search($value) is a separate method. 
            ...
            if (searchVal) {
                // Use search endpoint
                response = await fetch(`/api/mitra-api/search/${encodeURIComponent(searchVal)}`);
            } else {
                // Use standard index with potential page url
            ...
            if (result.success) {
                renderMitraList(result.data.data ? result.data.data : result.data); // Search might return array directly or wrapped
                renderPagination(result.data); // Metadata usually in result.data for pagination
            }
        } catch (error) {
            console.error(error);
    ...
    function renderMitraList(data) {
        ...
        if (!data || data.length === 0) {
            tbody.innerHTML = '<tr><td colspan="7" class="text-center p-5 text-muted fst-italic">Data mitra tidak ditemukan.</td></tr>';
            return;
        }
    ...
    async function deleteMitra(id) {
        ...
        try {
            ...
            if (dat.success) { alert('Berhasil dihapus'); loadMitraData(); }
    ...
    function renderPagination(meta) {
        ...
        meta.links.forEach(l => {
             ...
             c.insertAdjacentHTML('beforeend', `<li class="page-item ${!l.url?'disabled':''}"><a class="page-link border-0 mx-1 rounded shadow-sm fw-bold ${cls}" href="#" onclick="loadMitraData('${l.url}')">${l.label}</a></li>`);
        });
    }

    function resetFilter() {
        document.getElementById('filter-search').value = '';
        // Clear tomselect if exists, else value
        const ts = document.getElementById('filter-type').tomselect;
        if(ts) ts.clear(); else document.getElementById('filter-type').value = '';
        loadMitraData();
    }

    document.addEventListener('DOMContentLoaded', () => loadMitraData());
</script>
@endpush