...
<style>
    ...
    #plotTable thead th { border-bottom: 1px solid #e2e8f0; border-top: 1px solid #e2e8f0; background: #f8fafc; padding-top: 12px; padding-bottom: 12px; font-size: 11px; text-transform: uppercase; font-weight: 700; color: #64748b; letter-spacing: 0.5px; }
    #plotTable tbody td { font-size: 13px; color: #334155; padding-top: 12px; padding-bottom: 12px; border-bottom: 1px solid #f1f5f9; vertical-align: middle; }
    
    .badge-role { background: #fee2e2; color: #b91c1c; font-size: 11px; padding: 4px 8px; border-radius: 4px; border: 1px solid #fecaca; font-weight: 600; }
    .badge-office { background: #e0f2fe; color: #0284c7; font-size: 11px; padding: 4px 8px; border-radius: 4px; border: 1px solid #bae6fd; font-weight: 600; }
</style>
@endpush
    ...
    <div class="page-content">
        <div class="container-fluid">
             ...
             <div class="row align-items-center mb-4">
                <div class="col-md-7">
                    <h4 class="fw-bold text-dark mb-1">Plotting Otoritas User</h4>
                    <p class="text-muted small mb-0">Petakan user ke kantor dan role spesifik.</p>
                </div>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="d-flex align-items-center">
                                            <div class="bg-light rounded-circle border d-flex align-items-center justify-content-center text-primary fw-bold me-2" style="width:30px; height:30px; font-size: 11px;">
                                    ...
                                    <td>
                                        <span class="badge-office"><i class="fa fa-building me-1"></i> {{ $p->office_name }}</span>
                                    </td>
                                    <td>
                                        <span class="badge-role">{{ $p->role_name }}</span>
                                    </td>
                                    <td class="text-end pe-4">
                                        <button onclick="openPlotModal({{ $p->id }})" class="btn btn-sm btn-white border shadow-sm py-1 px-2 text-primary me-1" title="Edit Plotting">
                                            <i class="fa fa-pencil"></i>
                                        </button>
                                        <form action="{{ route('user_plots.destroy', $p->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Hapus akses plotting ini?')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-sm btn-white border shadow-sm py-1 px-2 text-danger" title="Hapus Akses">
...
<div class="modal fade" id="plotModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <form id="plotForm" action="{{ route('user_plots.store') }}" method="POST" class="modal-content border-0 shadow-lg">
            @csrf
            ...
            <div class="modal-body p-4">
                <div class="alert alert-soft-info border-0 small mb-3">
                    <i class="fa fa-info-circle me-1"></i> User yang diplot akan memiliki akses ke data kantor terkait sesuai role yang dipilih.
                </div>
                <div class="mb-3">
                 ...
                 <div class="mb-3">
                    ...
                    <select name="office_id" id="select-office" class="form-control" required placeholder="Cari Kantor...">
...
<script>
    ...
    document.addEventListener("DOMContentLoaded", function(){
        tomUser = new TomSelect("#select-user", { create: false, sortField: { field: "text", direction: "asc" } });
        tomOffice = new TomSelect("#select-office", { create: false, sortField: { field: "text", direction: "asc" } });
        tomRole = new TomSelect("#select-role", { create: false, sortField: { field: "text", direction: "asc" } });
    });
    ...
    async function openPlotModal(id = null) {
        ...
        if (id) {
            ...
            } catch(e) { console.error(e); alert('Gagal load data plot'); return; }
