            ...
            <div class="row mb-3">
                <div class="col-sm-12">
                    <div class="page-title-box d-md-flex justify-content-between align-items-center">
                        <div>
                            <h4 class="page-title fw-bold">Semua Kuitansi Pembelian</h4>
                            <p class="text-muted mb-0 small">Kelola kuitansi dan riwayat pembayaran kepada supplier/vendor.</p>
                        </div>
                        <ol class="breadcrumb mb-0">
                            <li class="breadcrumb-item"><a href="{{route('dashboard')}}">Dashboard</a></li>
                            <li class="breadcrumb-item active">Kuitansi Pembelian</li>
                        </ol>
                    </div>
                </div>
            </div>
                    ...
                    <div class="card shadow-sm border-0 rounded-4 overflow-hidden">
                        <div class="card-body p-4">
                            ...
                            <div class="row g-2 mb-4 p-3 bg-light rounded-3 align-items-end border">
                                <div class="col-md-4">
                                    <label class="form-label small fw-bold text-muted">Cari Kuitansi / Supplier</label>
                ...
                <form id="form-receipt" enctype="multipart/form-data">
                    <div class="row mb-4">
                        <div class="col-md-6">
                            ...
                            <select id="mitra_id" name="mitra_id" class="tom-select-init" placeholder="No Supplier Selected">
                    ...
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        <h6 class="fw-bold m-0"><i class="fa fa-file-invoice text-primary me-2"></i>Rincian Tagihan Pembelian</h6>
                        <button type="button" class="btn btn-outline-primary btn-sm fw-bold px-3" onclick="openInvoiceSelection()">
                            <i class="fa fa-plus-circle me-1"></i> Pilih Invoice Pembelian
                        </button>
                    </div>
                        ...
                        <table class="table table-hover align-middle mb-0" id="table-selected-invoices">
                            <thead class="bg-light">
                                <tr>
                                    ...
                                    <th class="small fw-bold text-muted py-3">Supplier</th>
                                    ...
                                    <th class="small fw-bold text-muted py-3 text-end">Jumlah Tagihan</th>
                    ...
                    <div class="row mt-4">
                        <div class="col-md-4">
                            <label class="form-label fw-bold text-muted small d-block">Bukti Pembayaran (Opsional)</label>
                            <div class="upload-area text-center border p-4 rounded-3 bg-light" id="upload-signature-area" style="cursor: pointer;" onclick="document.getElementById('signature_file').click()">
                                ...
                                <p class="mb-1 small fw-bold">Unggah Bukti</p>
                                <p class="mb-0 text-muted" style="font-size: 10px;">Maksimal 20MB (JPEG, PNG, PDF)</p>
                            </div>
                            <input type="file" name="signature_file" id="signature_file" class="d-none" accept="image/*,application/pdf">
...
<script>
    ...
    async function loadReceiptData(url = API_PAYMENT) {
        ...
        try {
            ...
            fetchUrl.searchParams.append('tipe_receipt', 'Purchase'); // Filter specifically for purchase
            ...
            if (result.success) {
                // Since our generic API doesn't have tipe_receipt filter yet, we'll manually filter if needed
                // But typically the API should handle this. For now let's assume it gets all within index
                // and we might need to filter by invoice.tipe_invoice == 'Purchase'
                const filteredData = result.data.data.filter(item => item.invoice?.tipe_invoice === 'Purchase');
    ...
    function renderReceiptList(data) {
        ...
        accordion.innerHTML = '';
        if (data.length === 0) {
            accordion.innerHTML = '<div class="text-center p-5 text-muted">Belum ada data kuitansi pembelian.</div>';
            return;
        }
                ...
                <div class="accordion-header bg-white">
                    <div class="d-flex align-items-center">
                        ...
                        <div class="col-fixed-aksi text-center pe-2">
                             <a href="{{ url('purchase-receipt/print') }}/${item.id}" target="_blank" class="btn btn-sm btn-light border shadow-none"><i class="fa fa-print"></i></a>
                        </div>
                    </div>
                </div>
                            ...
                            <table class="table table-sm table-hover mb-0 align-middle">
                                ...
                                <tbody>
                                    <tr>
                                        ...
                                        <td>${item.invoice?.tipe_invoice || 'Purchase Invoice'}</td>
    ...
    async function submitReceipt(action) {
        ...
        try {
            ...
            for (const inv of selectedInvoices) {
                if (inv.bayar > 0) {
                    const payload = { invoice_id: inv.id, nomor_pembayaran: 'PYP/' + Date.now() + '/' + inv.id, tgl_pembayaran: tgl, metode_pembayaran: 'Cash', jumlah_bayar: inv.bayar, akun_keuangan_id: akunId, ref_no: refNo, catatan: catatan };
                    const res = await fetch(API_PAYMENT, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
                    results.push(await res.json());
                }
            }

            if (results.every(r => r.success)) { alert('Kuitansi pembelian berhasil disimpan.'); location.reload(); }
